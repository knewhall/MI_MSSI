...
#     print(dt)
    ...
    for j in range(1,tstep-1): #this is the "final" time as we creep across the segment
        ...
        func_vals_x = np.multiply(vecfun(U[j],U[0:(j+1)],V[j],V[0:(j+1)],tpts[j],tpts[0:(j+1)],mu,phi,nu)
                                  ,-prefactorU)  
        
        prefactorV = V[0:(j+1)]-np.array(V[j])
        func_vals_y = np.multiply(vecfun(V[j],V[0:(j+1)],U[j],U[0:(j+1)],tpts[j],tpts[0:(j+1)],mu,phi,nu)
                                  ,-prefactorV)
        
        U_int = scipy.integrate.simps(func_vals_x,tpts[0:(j+1)],axis=-1, even='avg')*dt
        V_int = scipy.integrate.simps(func_vals_y,tpts[0:(j+1)],axis=-1, even='avg')*dt
